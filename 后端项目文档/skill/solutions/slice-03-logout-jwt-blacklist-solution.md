...
Auth 面板会自动生成请求头，因此 Headers 面板不要再手工添加 `Authorization`。若两处同时配置，可能产生重复 Header 或因两份 token 不一致导致难以定位的 401。`selfmark.yaml` 中的 `bearerAuth` 是标准 OpenAPI 安全定义，导入 Apifox 后应由 Auth 面板接管。
...
- Redis 不可用时认证链路不绕过黑名单，避免已登出 token 被错误放行；当前由统一异常处理返回 500。
