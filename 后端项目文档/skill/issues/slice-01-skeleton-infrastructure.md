...
骨架需要：
- Maven `pom.xml` 声明 Spring Boot 3.x、MyBatis-Plus、MySQL 驱动、Redis 客户端（Spring Data Redis）、JWT 库、BCrypt、Lombok、测试库（Spring Boot Test + Testcontainers + MockMvc）
...
- 拦截器注册：所有 `/api/**` 受保护，`/api/auth/register`、`/api/auth/login`、`/api/health` 放行；接口契约由 Apifox 管理
...
- [x] `mvn spring-boot:run` 能成功启动应用，用户已确认本机依赖启动后日志无配置或连接错误
- [ ] 应用启动时能成功连接 MySQL 和 Redis（需在本机 Docker 环境复验；连接失败应启动失败并报清晰错误）
- [x] 未携带 token 访问 `/api/**`（非放行路径）返回 `{code:401, msg:"未登录", data:null}`
- [x] 携带格式错误的 token 返回 `{code:401, msg:"token 无效", data:null}`
- [x] 携带过期 token 返回 401
- [x] 携带合法 token（用 JWT 工具手动签发一个测试 token）能访问 `/api/**`，请求上下文里能取出 userId
- [x] 参数校验失败（如 POST body 缺字段、@NotBlank 触发）返回 `{code:400, msg:"...", data:null}` 而非 Spring 默认错误格式
- [x] 手动抛 BusinessException(409, "冲突") 返回 `{code:409, msg:"冲突",data:null}`
- [x] 未捕获的 NullPointerException 等返回 500 而非 Spring 默认错误页
- [ ] Apifox 导入维护的 OpenAPI 文件后，`GET /api/health` 能在云端默认模块中读取和调用；需由用户在 Apifox 完成导入/导出
- [x] 访问 `/api/health` 返回 200（MockMvc 已验证）
- [x] CORS 预检 OPTIONS 请求能正常返回（自动化测试已验证，支持 Flutter Web）
